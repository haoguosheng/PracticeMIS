...
//    private boolean inputed; //检查本击是否已经提交
    ...
    public void submitPracNote() {
        ...
        if (tempc.after(Calendar.getInstance())) {
            ...
            if (hasSubmitted) {
                ...
                practiceNote.setEnterid(Integer.parseInt(seDao.getIdListHandlerRunner("select * from stuentrel" + loginUser.getSchoolId() + " where stuno='" + getUser().getUno() + "'").get(0)));
                practiceNote.setPositionid(positionId);
                practDao.executUpdate("insert into practicenote" + loginUser.getSchoolId() + "(detail, submitdate, enterid, positionId, stuno) values('" + practiceNote.getDetail() + "', " + practiceNote.getSubmitdate() + ", " + practiceNote.getEnterid() + ", " + practiceNote.getPositionid() + ", '" + practiceNote.getStuno() + "')");
    ...
    public List<Practicenote> getSubmittedNoteList() {
        if (null == submittedNoteList) {
            submittedNoteList = practDao.getBeanListHandlerRunner("select * from practicenote" + loginUser.getSchoolId() + " where userno='" + this.getUser().getUno() + "'",practiceNote);
        }
    ...
    public String directToNote() {
        ...
        Date requestDate = new Date(context.getExternalContext().getRequestParameterMap().get("submitDate"));
        Iterator<Practicenote> it = submittedNoteList.iterator();
        while (it.hasNext()) {
        ...
        if (null == this.positionMap || this.positionMap.isEmpty()) {
            ...
            if (ent.size() <= 0) {//还没有选择企业呢，跳转选择企业
                ...
                for (int i = 0; i < ent.size(); i++) {
                    List<Enterstudent> reqStu = entersDao.getBeanListHandlerRunner("select * from enterstuent" + loginUser.getSchoolId() + " where enterId=" + ent.get(i).getEnterid(), entStu);
    ...
    public User getStudentUser() {
        ...
        studentUser = userDao.getBeanListHandlerRunner("select * from student" + sessionuser.getSchoolId() + " where uno='" + studentUserno + "'", this.getUser()).get(0);
        return studentUser;
    }
